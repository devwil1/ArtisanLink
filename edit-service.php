...
// Get service details
$service = [];
$sql = "SELECT * FROM services WHERE id = ? AND professional_id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($title_err) && empty($description_err) && empty($category_err) && empty($price_err)) {
        ...
        $sql = "UPDATE services SET title = ?, description = ?, category = ?, price = ?, pricing_type = ? WHERE id = ? AND professional_id = ?";
         
        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "sssdsii", $title, $description, $category, $price, $pricing_type, $service_id, $_SESSION["id"]);
...
<head>
    <meta charset="UTF-8">
    <title>Edit Service - Artisan Link</title>
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header bg-primary text-white">
                        <h4 class="mb-0"><i class="fas fa-edit me-2"></i>Edit Service</h4>
                    </div>
